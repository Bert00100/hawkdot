...
# Worker: processo, conexão e contexto (#33)

**Formato do processo**: script Node standalone (`src/worker/run.ts`, ver `npm run worker`), não uma rota HTTP acionada por cron externo. Mesma razão do guard de sessão (M3): rotas/Proxy do Next não são o lugar certo para lógica de longa duração ou agendada. Um processo próprio, com seu próprio loop de polling, é mais simples de raciocinar para o MVP do que expor um endpoint com segredo compartilhado para um cron externo bater. Reavaliar se um dia precisar de múltiplas réplicas coordenadas por infraestrutura externa (ECS/K8s CronJob, etc.) — o mecanismo de reserva do #34 (`SKIP LOCKED`) já foi desenhado para tolerar múltiplas instâncias rodando ao mesmo tempo.

**Conexão separada da API**: `src/worker/database.ts` conecta como `hawkdot_worker_login`, nunca compartilha client com `src/config/database.ts`. `DATABASE_URL_WORKER`/`DATABASE_URL_WORKER_TEST` ficam **opcionais** no `envSchema` principal (`src/config/env.ts`) de propósito — só o worker precisa delas, e a API não deveria falhar o boot por uma variável que não usa. `workerDatabaseUrl()` valida a presença no momento em que o worker de fato conecta, não a API.
...
# Motor de entrega de notificações e o status `skipped` (#41)

`processPendingEvents(tx, organizationId)` (`src/notifications/delivery-engine.ts`) roda com o role da API (`hawkdot_app`), dentro do `withTenant()` do usuário/organização dona do evento — nunca no worker. Motivo: o worker (`hawkdot_worker`) não tem `GRANT` em `notification_rules`/`notification_channels`/`notification_rule_channels` de propósito (#38: ele só emite eventos, não sabe nada sobre regras). Quem casa evento → regra → canal e decide a entrega tem que ser a camada que já enxerga essas tabelas.

**Grant adicional necessário** (mudança aditiva no `.sql`, mesmo padrão do #12): `hawkdot_app` tinha só `SELECT` em `notification_deliveries` (parecia intencional, mas nenhuma outra camada tinha `INSERT` também — provável lacuna do desenho original). Sem `INSERT`, a própria criação da entrega falhava com 42501. Adicionado `GRANT SELECT, INSERT ON hawkdot.notification_deliveries TO hawkdot_app` — sem `UPDATE`: mudar o status de uma entrega (`pending` → `sent`/`failed`) é responsabilidade do adapter de entrega (#42), não desta função.

**"Evento novo" é definido por ausência total de entrega, não por uma coluna de "processado"**: cada evento, ao ser processado, gera exatamente uma linha de entrega por combinação (regra casada × canal habilitado) — nunca uma entrega parcial. Por isso `NOT EXISTS` (via `notification_deliveries: { none: {} }`) já basta para saber se um evento ainda não foi avaliado; não precisou de coluna nova.

**Por que existe o status `skipped` no enum**: é exatamente para o caso de cooldown. Quando o evento casa com uma regra mas o cooldown ainda está ativo, o motor cria a entrega mesmo assim, só que com `status = 'skipped'` em vez de `'pending'` — isso marca o evento como processado (satisfaz a regra de "evento novo" acima) sem de fato despachar nada. Sem isso, o mesmo evento seria reavaliado a cada chamada futura, e o resultado do cooldown mudaria dependendo de quando a função roda, não de quando o evento aconteceu.

**Cooldown duplo — regra vs. monitor**: quando a regra é específica de um monitor (`monitor_id` preenchido) e o evento também carrega `monitor_id`, o cooldown efetivo é o **maior** entre `notification_rules.cooldown_seconds` e `monitors.notification_cooldown_seconds`. Decisão deliberada: entre os dois limiares configurados por pessoas diferentes (quem criou a regra, quem configurou o monitor), erra para o lado de notificar **menos**, não mais. Regra geral (sem `monitor_id`) usa só o cooldown da própria regra — não há limiar de monitor para comparar. O cooldown em si é medido contra o `sent_at` da última entrega com `status = 'sent'` para o mesmo par regra+canal — `'skipped'`/`'failed'`/`'pending'` não contam, porque não representam um envio de fato.

# Adapters de entrega e retry com backoff (#42)

`src/notifications/adapters/` define uma interface `ChannelAdapter<TConfig, TSecret>` comum (`send({config, secret, message, timeoutMs}) -> ChannelAdapterResult`) — um canal novo (email, slack) implementa isso e entra no `if/else` de `send()` em `dispatch-pending.ts` sem tocar no motor de entrega (#41) nem no loop de retry. Hoje: `telegram.adapter.ts` (Telegram Bot API `sendMessage`) e `webhook.adapter.ts` (POST JSON, `Authorization: Bearer <secret>` opcional quando o canal tem `credential_id`).

**Segredo nunca aparece em `last_error`**: a URL do Telegram carrega o bot token embutido (`.../bot<token>/...`), e exceptions de `fetch` às vezes ecoam a URL inteira — por isso todo adapter passa a mensagem de erro por `sanitizeError()` (`adapters/types.ts`) antes de devolver, que substitui qualquer ocorrência literal do segredo por `[REDACTED]`. Mesma função nos dois adapters, mesmo em erro de rede (`catch`), não só em resposta HTTP de erro.

**`dispatchPendingDeliveries` roda no mesmo role da API** (`hawkdot_app`), pela mesma razão do motor de entrega (#41): precisa ler `credentials`/`notification_channels`/`telegram_channel_configs`, que o worker não tem grant. Precisou de `GRANT UPDATE` adicional em `notification_deliveries` (só tinha `SELECT, INSERT` desde #41) — aplicado de forma aditiva no schema.sql e no banco de dev/teste.

**Retry**: backoff exponencial (`30s * 2^(attempt_count-1)`, teto de 1h) via `next_attempt_at`; a consulta de elegibilidade (`findDispatchableDeliveries`) usa o mesmo filtro do índice parcial `notification_deliveries_pending_idx` (`status IN (pending, failed)`) mais `attempt_count < 20` e `next_attempt_at` vencido ou nulo. Ao atingir a 20ª tentativa (o teto do CHECK `notification_attempt_count`), `next_attempt_at` vira `null` — a entrega para de aparecer na consulta de elegíveis sem precisar de um status novo (`failed` já é o status; só o `next_attempt_at` nulo marca "sem mais tentativa agendada").

**Isolamento entre canais**: cada delivery é processada num `try/catch` individual dentro do loop de `dispatchPendingDeliveries` — uma falha inesperada de um adapter (exceção não tratada, bug) vira uma entrega `failed` normal, nunca aborta o processamento das demais entregas do lote.

# audit_logs em ações sensíveis (#43)

`recordAudit()` (`src/lib/audit/record-audit.ts`) é chamado pelos controllers depois da operação principal, dentro da mesma transação do `withTenant()` — credenciais (create/delete), organização (update), convite/troca de papel/remoção de membro, monitores (create/delete) e reconhecimento de incidente (`acknowledgeIncidentController`, feature nova desta issue, já que não existia nenhum endpoint de usuário para incidentes antes — só o worker os cria/resolve). IP e user agent vêm de `extractRequestMeta(request)`, chamado na camada de rota (única camada que conhece `Request`) e passado como dado simples (`{ipAddress, userAgent}`) para o controller — nunca o objeto `Request` em si, para não quebrar a regra de camadas do AGENTS.md.

**Sétimo caso do "RETURNING quebra com RLS assimétrico"** (mesma família do #14/#24, ver seção "Postgres exige visibilidade de SELECT..."): `audit_logs_admin_select` restringe leitura a owner/admin, mas `audit_logs_member_insert` permite que **qualquer membro ativo** grave (`actor_user_id = current_user_id() AND is_organization_member(...)`, sem checar papel). Um `operator` criando uma credencial aciona `recordAudit()` como parte da mesma ação — e `.create()` do Prisma sempre faz `INSERT...RETURNING`, que exige visibilidade via alguma policy de `SELECT`. Como `operator` não é owner/admin, `audit_logs_admin_select` não o alcança, e o insert falhava com 42501 mesmo satisfazendo o `WITH CHECK` do insert. Mesma solução de sempre: `$executeRaw` sem `RETURNING` (`createAuditLog`/`createWorkerAuditLog`).

**Nunca segredo em `before_data`/`after_data`**: para `credentials`, só `name`/`credential_type` (nunca `secret`/`encrypted_secret`/`secret_reference`) — o payload de auditoria é montado à mão em cada controller, não por um serializer genérico, exatamente para evitar que um campo sensível novo vaze por acidente ao ser adicionado à entidade auditada no futuro.

**Worker grava com `actor_user_id` nulo, sempre**: `src/worker/audit-log.model.ts` tem sua própria função (`createWorkerAuditLog`, também via `$executeRaw` sem `RETURNING` — `hawkdot_worker` só tem `GRANT INSERT` em `audit_logs`, nem `SELECT`) chamada em `incident-state-machine.ts` na abertura e resolução de incidente (`incident.opened`/`incident.resolved`). `audit_logs_worker_insert` só aceita `actor_user_id IS NULL` — gravar diferente disso viola a policy, então não há como reutilizar `recordAudit()` (que sempre grava `session.userId`) do lado do worker.

**Leitura**: `GET /api/audit-logs`, restrito a owner/admin (`audit_logs_admin_select` no banco + `requireRole` na aplicação pela mesma mensagem-de-erro-melhor de sempre), paginado com `dto.paginacao`.
